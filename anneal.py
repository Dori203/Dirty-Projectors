...
class Annealer(object):
    ...
    def anneal(self, statistics):
        ...
        progress_bar_iterator = tqdm(iterable=range(self.steps),
                                     bar_format='{desc}: {percentage:3.0f}% |{bar}| {n_fmt}/{total_fmt}{postfix}',
                                     desc='simulated annealing')
        for i in progress_bar_iterator:
            step += 1
            T = self.Tmax * math.exp(Tfactor * step / self.steps)
            dE, neighbors = self.move()
            if dE is None:
                E = self.energy()
                dE = E - prevEnergy
            else:
            ...
            if self.updates > 1:
                if (step // updateWavelength) > ((step - 1) // updateWavelength):
                    ...
                    trials, accepts, improves = 0, 0, 0
            progress_bar_iterator.set_postfix_str('loss=%.2f' % self.best_energy)
            self.losses.append(self.best_energy)

            # statistics:
            # add current state data
            statistics.add_current_state_data(self.state)
            # add neighbors data
            statistics.add_neighbors_data(neighbors)
        ...
        return self.best_state, self.best_energy, statistics
